...
class LiveRecipeCreate extends Component
{
    ...
    public function setFirstResult()
    {
        ...
        $this->foodSearch = $food->food_name;
        $this->selectedFoodId = $food->id;
    }

    public function setFood($id, $name)
    {
        $this->selectedFoodId = $id;
        $this->foodSearch = $name;
    }

    public function render()
    {
        if ($this->showLoadRecipeModal) {
            $this->foods = Food::has('recipes')->with('recipes')
            ->when(strlen($this->foodSearch) > 2 ? $this->foodSearch : false, function ($query) {
                $query->where('food_name', 'like', '%' . $this->foodSearch . '%');
            })
            ->orderBy('food_name')
            ->get();
