...
class LiveWalkIn extends Component
{
    ...
    public function setFirstResult()
    {
        ...
        $this->inhouse->customer_id = $customer->id;
    }

    public function setCustomer($id, $name)
    {
        $this->customerSearch = $name;
        $this->inhouse->customer_id = $id;
    }

    public function render()
    {
        $this->customers = Customer::select('id', 'customer_name')
        ->when(strlen($this->customerSearch) > 2 ? $this->customerSearch : false, function ($query) {
