                ...
                <div @click.outside="showLoadRecipeModal = false"
                    ...
                    @if ($showLoadRecipeModal)
                        <div class="flex-1 px-4 py-5 text-sm">
                            ...
                            <x-combobox class="col-span-4" wire:model="foodSearch" for="customers" label="Select a Food">
                                @foreach ($foods as $food)
                                    <li wire:key="{{ $food->id }}"
                                        @click="$wire.setFood({{ $food->id }}, '{{ $food->food_name }}').then(show = false)"
                                        class="relative cursor-default hover:text-white hover:bg-primary select-none py-2 pl-3 pr-9 text-gray-900"
                                        role="food" tabindex="-1">
                                        <span class="block truncate">{{ $food->food_name }}</span>
                                    </li>
                                @endforeach
                            </x-combobox>
