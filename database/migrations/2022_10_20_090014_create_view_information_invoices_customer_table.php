...
return new class () extends Migration {
    ...
    {
        return <<<SQL
			CREATE VIEW `view_information_invoice_customers` as
			SELECT
				1 as group_no,
				inhouses.id,
			...
			SELECT
				...
				null as id,
				inhouse_id as inhouse_id,
				'Inhouse Services' as description,
				concat('Staff x ', count(ss.id)) as reference,
				service_staff_rate as price,
				sum(inhouse_services.session_hours) as qty,
				sum(inhouse_services.session_hours * service_staff_rate) as amount
			from inhouse_services
			left join service_staff ss on ss.id = inhouse_services.service_staff_id
			group by inhouse_id, service_staff_rate
			union
			SELECT
				1 as group_no,
    ...
    {
        return <<<SQL
			DROP VIEW IF EXISTS `view_information_invoice_customers`;
		SQL;
    }
};
